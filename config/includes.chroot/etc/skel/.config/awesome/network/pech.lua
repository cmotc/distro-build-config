...
--                work = {nwork, terminal .. " -e " .. COMMAND_WIFI_CONNECT .. " " .. w[1] }
                work = {nwork, function() command_connect_wifi_network(w[1]) end }
                table.insert(wifi_list, work)
            end
            ud = {"Toggle Wireless Interface On/Off", terminal .. " -e sudo /usr/bin/iface-wrapper " .. iface}
...
--            face = {iface, terminal .. " -e " .. COMMAND_LIST_WIFIS}
            face = {iface, function() get_area_wifi() end}
            table.insert(networks, face)
        else
            face = {iface, terminal .. " -e sudo /usr/bin/iface-wrapper " .. iface}
            table.insert(networks, face)
        end
    end
    return networks
